/**
 * ДАННЫЕ ГЛОССАРИЯ — 119 терминов, отдельно от кода сева.
 *
 * Вынесены из `prisma/seed-glossary.ts` 18.09.2026 (заход 7.211) по
 * одной причине: сторож `check:locale-mixing` обязан читать эти строки
 * в CI, где боевой базы нет, а сам файл сева импортирует `src/lib/db`
 * — то есть клиент Prisma. Импорт данных не должен тянуть за собой
 * соединение с базой; это тот же довод, по которому существует
 * `prisma/stories-data.ts`.
 *
 * Правила содержимого — в шапке `prisma/seed-glossary.ts`.
 */
import type { GlossaryCategory, GlossaryExample } from "../src/lib/glossary";
