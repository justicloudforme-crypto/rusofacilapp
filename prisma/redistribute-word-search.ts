 ...
 *
 * Чего НЕ делает: не удаляет строк, не трогает CROSSWORD, не меняет
 * правило бесплатности, не добавляет и не убирает ни одного URL в
 * sitemap.xml — набор там выведен из констант free-tier.ts, а не из базы.
 *
 * Что делает начиная с 05.09.2026 (PROGRESS 7.109) и не делал раньше:
 * ТРОГАЕТ бесплатные рунги 1–10. Порция 5 — это они и есть, и потому она
 * единственная, после которой sitemap.xml перестаёт быть побайтово тем же
 * файлом: <lastmod> берётся из updatedAt, и у 26 строк он сдвинулся.
 * Разрешение не сплошное: список бесплатных источников перечислен в
 * `PORTION_5_SOURCES`, и сверка перед записью (prisma/verify-density-rungs.ts)
 * по-прежнему останавливает прогон на любом бесплатном рунге вне этого
 * списка и на любом ХВОСТЕ, попавшем в бесплатную десятку.
 *
