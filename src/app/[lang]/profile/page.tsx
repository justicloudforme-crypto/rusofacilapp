...
function computeBadgeProgress(
  ...
  if (def.id.startsWith("streak-")) {
    /**
     * ЧИСЛО И ПОДПИСЬ — ОБ ОДНОМ (заход 7.202, часть 5).
     *
     * ЧТО СНЯЛ ВЛАДЕЛЕЦ 16.09.2026. У Vasya приветствие называет серию
     * 3, а «Целая неделя — 7 дней занятий подряд» показывает 5/7 и
     * «Месяц упорства» 5/30. У сотрудника текущая серия 5, лучшая 10 —
     * а «Месяц упорства» 10/30 и «Сто дней» 10/100.
     *
     * ЧТО ЭТО СЧИТАЕТ, ЧИСЛОМ. Оба набора совпадают с `longestStreak`
     * знак в знак (5 и 10) и не совпадают с `currentStreak` (3 и 5).
     * Заморозки, пропуски и «дни занятий» тут ни при чём вовсе:
     * числитель — это ЛУЧШАЯ серия, ровно тот же счётчик, по которому
     * значок и выдаётся (`src/lib/badges/index.ts:49…52`).
     *
     * ЧТО ВЫБРАНО И ПОЧЕМУ. Не «считать по текущей серии», а назвать то,
     * что считается. Числитель обязан быть ТЕМ ЖЕ, что решает выдачу:
     * покажи здесь текущую серию — и человек, у которого лучшая 29, а
     * текущая 2, увидит «2/30» за день до значка. А выданное значком не
     * отнимается ни в одном из двух вариантов: выдача стоит на
     * `longestStreak`, и её эта правка не трогает вовсе.
     *
     * ЦЕНА. Подпись стала длиннее на два слова в обеих локалях
     * («лучшая серия: 5/7 дней» / «mejor racha: 5/7 días»), и на узкой
     * плитке «Что дальше» она переносится в две строки. Больше не
     * изменилось ничего: ни одно число, ни один выданный значок, ни
     * порядок плиток (`ratio` считается прежним делением).
     */
        ...
        <section className="mt-8 flex flex-col gap-6">
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {/* ЗАХОД 7.202, ЧАСТЬ 2. ОДНО ПРАВИЛО НА ОБЕ ВКЛАДКИ.

                  ЧТО СНЯЛ ВЛАДЕЛЕЦ 16.09.2026: у сотрудника на «Обзоре»
                  стояло «Бесплатный план», а на вкладке «Подписка» —
                  «Полный доступ». Одна страница, один человек, две
                  взаимоисключающие надписи.

                  ПРИЧИНА, И ОНА НЕ В ОПЕЧАТКЕ. Долг 221 (7.200) завёл
                  признак `staffAccess` и применил его РОВНО В ОДНОМ месте
                  — в бейдже вкладки «Подписка». Здешний бейдж остался на
                  прежнем правиле, у которого выбор был из трёх: пожизненный,
                  активная подписка, иначе «бесплатный». Роли в этом выборе
                  не было вовсе, и сотрудник без строки `Subscription`
                  попадал в «иначе».

                  Теперь `staffAccess` стоит ПЕРВЫМ на обеих вкладках, и
                  цвет бейджа берётся тем же выражением, что там. Условие
                  «и активной подписки нет» внутри самого `staffAccess`:
                  сотрудник, купивший подписку, обязан видеть свою подписку,
                  а не роль. Держит `npm run check:cabinet-plan-truth`. */}
              <span
            ...
            {/* ЗАХОД 7.202, ЧАСТЬ 2, вторая половина той же правды.
                Бейдж выше говорил сотруднику «Бесплатный план», а эта
                кнопка — «откройте всё»: тот же признак роли не спрашивали
                и здесь. Человеку с полным доступом предлагать открыть
                доступ нечего, и вкладка «Подписка» это уже знает
                (`staffAccessNotice`). Вычитается ровно `staffAccess` —
                сотрудник, купивший подписку сам, снова идёт по общим
                ветвям ниже. */}
            {nativeShell || staffAccess ? null : !isActive ? (
              <Link
                href={`/${lang}/pricing`}
