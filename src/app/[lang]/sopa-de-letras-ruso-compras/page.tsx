...
// the page answers a Spanish search intent ("sopa de letras de compras en
// ruso") that has no Russian-language equivalent, so /ru 404s rather than
// serving the same Spanish text at a second URL.
const LANDING = getTopicLanding("compras")!;

export async function generateMetadata({
  params,
}: PageProps<"/[lang]/sopa-de-letras-ruso-compras">): Promise<Metadata> {
...
export default async function Page({
  params,
}: PageProps<"/[lang]/sopa-de-letras-ruso-compras">) {
  const { lang } = await params;
  if (lang !== "es") notFound();
