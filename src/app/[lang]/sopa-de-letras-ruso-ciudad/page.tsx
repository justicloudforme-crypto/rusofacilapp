...
// the page answers a Spanish search intent ("sopa de letras de la ciudad en
// ruso") that has no Russian-language equivalent, so /ru 404s rather than
// serving the same Spanish text at a second URL.
const LANDING = getTopicLanding("ciudad")!;

export async function generateMetadata({
  params,
}: PageProps<"/[lang]/sopa-de-letras-ruso-ciudad">): Promise<Metadata> {
...
export default async function Page({
  params,
}: PageProps<"/[lang]/sopa-de-letras-ruso-ciudad">) {
  const { lang } = await params;
  if (lang !== "es") notFound();
