  ...
  if (target) {
    await db.subscription.update({
      ...
      data: {
        ...
        /**
         * ПОДПИСЬ НАЗЫВАЕТ ПОСЛЕДНЕЕ СОБЫТИЕ, А НЕ СОЗДАТЕЛЯ СТРОКИ — ДОЛГ 102.
         *
         ...
         * продлевает любую живую непремиальную строку, не глядя на её план.
         * Код, погашенный поверх живой ручной выдачи, продлит строку
         * `"manual"` и подписи не сменит; ручная выдача поверх кода оставит
         * `"access_code"`».
         *
         * ЦЕНА БЫЛА НЕ ТОЛЬКО В ПОДПИСИ, и это главное, что нашёл разбор
         * 19.09.2026. `isGrantSubscription` (src/lib/subscription-grant.ts)
         * судит по ДВУМ условиям: план не из числа оплаченных И подписки в
         * кассе нет. У покупки через OXXO подписки в кассе нет по
         * построению — денег касса берёт разово, — поэтому оплаченный
         * месяц, легший продлением на живую строку `"manual"`, оставался
         * подписан «выдано вручную». Следствий два, и оба видны человеку:
         * его платёж не попадал в «Historial de pagos», а кнопка «отменить
         * подписку» показывалась там, где отменять нечего. То есть это не
         * вкусовщина о слове на экране, а неверная запись о деньгах.
         *
         * ПОЧЕМУ ЭТО БЕЗОПАСНО ДЛЯ ДОСТУПА. Строка-цель отобрана условием
         * `isPremiumPlan(row.plan) === premiumGrant` — то есть новое
         * значение всегда той же премиальности, что и старое. Решение о
         * доступе (`tierOfStoredSubscription`) читает ровно эту
         * премиальность, и переписать его этой строкой нельзя по
         * построению.
         */
        plan,
