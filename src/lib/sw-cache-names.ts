...
export function pageCacheNames(fingerprint: string) {
  return {
    ...
    /**
     * ЛИСТЫ СТИЛЕЙ СОХРАНЁННЫХ КОПИЙ — ЗАХОД 7.233, СТРОКА 314.
     *
     * Копия 7.230 читается без сети только вместе со своими стилями, и
     * до этого захода они брались из precache — то есть из кеша, которым
     * распоряжается не копия, а сборка. Один и тот же вопрос «можно ли
     * показать» давал разные ответы, потому что между ответами менялся
     * precache.
     *
     * СВОЙ КЕШ, А НЕ МЕСТО В `content`, И ЭТО НЕ ОПРЯТНОСТЬ. У
     * `content` и `section` потолки объявлены В ЗАПИСЯХ (36 и 8), и
     * записи эти — ДОКУМЕНТЫ: «восемь корней разделов» значит восемь
     * каталогов, а не шесть каталогов и два служебных файла. Положи
     * стили туда — и потолок начал бы вытеснять урок ради css. Поймано
     * пробой `e2e/sw-cache-budget.spec.ts` («корни разделов: 5 записей
     * против объявленных 4»), а не рассуждением.
     *
     * ОТПЕЧАТОК СБОРКИ В ИМЕНИ ОБЯЗАТЕЛЕН: лист стилей принадлежит своей
     * сборке так же, как и разметка, которая на него ссылается, и жить
     * дольше неё не должен. Скачанное (кнопкой) устроено иначе — там
     * листы лежат в `downloads`, без отпечатка, потому что и обещание
     * там другое.
     */
    sheets: `${PAGE_CACHE_PREFIX}-sheets-${fingerprint}`,
  };
}
