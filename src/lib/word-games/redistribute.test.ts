...
describe("the redistribution manifest", () => {
  it("names only paid, unthemed rungs — in every round", () => {
    ...
    // записана 03.09) и двадцать рунгов пилота порции 2 (7.106,
    // записан 04.09) — двести четыре. Длина всего манифеста НЕ
    ...
    expect(DENSITY_SPLITS.filter((s) => s.applied)).toHaveLength(204);
    expect(DENSITY_SPLITS.length).toBeGreaterThanOrEqual(204);
