...
describe("the redistribution manifest", () => {
  it("names only paid, unthemed rungs — in every round", () => {
    ...
    // коридора (7.86) и сто двадцать четыре рунга порции 1 (7.101,
    // записана 03.09) — сто восемьдесят четыре. Длина всего манифеста НЕ
    ...
    // строк (PROGRESS 7.101), и оно же три дня держало порцию 1
    // непомеченной, хотя прод её уже носил (7.105).
    expect(DENSITY_SPLITS.filter((s) => s.applied)).toHaveLength(184);
    expect(DENSITY_SPLITS.length).toBeGreaterThanOrEqual(184);
