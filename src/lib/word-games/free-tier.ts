...
export const EXTRA_FREE_WORD_GAME_RUNGS: readonly WordGameRungRef[] = [
  // Хвосты трёх делившихся рунгов порции 5 (PROGRESS 7.109, 7.110).
  //
  // 05.09.2026 порция 5 разгрузила бесплатную десятку, и три её рунга
  // не поместились в одну доску: половина слов уехала на новую строку в
  // конце лестницы. Источники — тематические, бесплатные и
  // проиндексированные страницы; хвосты по правилу номера оказались
  // платными и вне sitemap. Итог: страница обещала 24 слова, показывала
  // 11, а остальные 13 лежали за пейволлом на странице, о которой никто
  // не знал.
  //
  //   B1/9  «наука» 24 → 11 слов, остальные 13 → B1/668
  //   B1/10 «искусство и досуг» 26 → 10 слов, остальные 16 → B1/669
  //   B2/10 «абстрактные понятия» 26 → 12 слов, остальные 14 → B2/405
  //
  // Решение владельца: ни одно слово бесплатной страницы не остаётся за
  // пейволлом — хвосты открываются и входят в sitemap. Вместо трёх
  // проиндексированных страниц становится шесть.
  { type: "WORD_SEARCH", level: "B1", sequence: 668 },
  { type: "WORD_SEARCH", level: "B1", sequence: 669 },
  { type: "WORD_SEARCH", level: "B2", sequence: 405 },
];
