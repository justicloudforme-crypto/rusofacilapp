...
/**
 * Страницы, чей СЕРВЕРНЫЙ HTML печатает индекс бесплатных рунгов.
 *
 * Замер на живом проде 05.09.2026, после мержа 7.110: все 166 бесплатных
 * игровых URL (83 строки × две локали) стоят в sitemap, разрешены
 * robots.txt и имеют входящую ссылку из серверного HTML — 166 из 166,
 * глубина от корня локали 2. Источников ровно три, и каждый называет все
 * 83 рунга своей локали: `/es/word-games`, `/ru/word-games` и
 * `/es/juegos-para-aprender-ruso`. Для 73 из 83 рунгов каждой локали это
 * ЕДИНСТВЕННЫЙ источник: плитки самого пикера сервер печатает только для
 * пары по умолчанию (WORD_SEARCH/A1) — на `/es/word-games` их 198 из 281
 * ссылки, остальные 83 даёт этот индекс.
 *
 * Отсюда правило, ради которого список выделен: у страницы-источника
 * `<lastmod>` обязан быть, и он обязан двигаться тогда же, когда меняется
 * набор бесплатных рунгов. Иначе выходит то, что и вышло 04.09: три
 * хвоста стали бесплатными, HTML всех трёх источников изменился, а
 * sitemap не сказал об этом ни слова — их записи `<lastmod>` не имели
 * вовсе. Единственный рычаг переобхода страницы, которая раздаёт 83
 * ссылки, молчал.
 *
 * Список нарочно перечислением, а не поиском по файловой системе в
 * рантайме: он виден в дифе, а совпадение с настоящими страницами
 * сверяется тестом (free-index.test.ts читает `src/app/[lang]` и требует
 * равенства в обе стороны).
 */
export const FREE_INDEX_PATHS_EVERY_LOCALE = ["/word-games"] as const;

/** То же, но у страницы нет русского близнеца (см. её page.tsx). */
export const FREE_INDEX_PATHS_ES_ONLY = ["/juegos-para-aprender-ruso"] as const;

/**
 * Дата для `<lastmod>` страницы-источника: самая поздняя `updatedAt`
 * среди строк, которые правило зовёт бесплатными.
 *
 * Честность даты держится на двух вещах. Первая — считается по тем же
 * строкам, которые страница и печатает: строка, которую правило
 * бесплатной не считает, дату не двигает, даже если она новее всех
 * (позитивный контроль в тесте подсаживает ровно такую). Вторая — при
 * отсутствии дат возвращается `undefined`, и вызывающий не пишет
 * `<lastmod>` вовсе: выдуманная дата на неизменившемся URL хуже
 * молчания, потому что недостоверный `lastmod` игнорируется целиком
