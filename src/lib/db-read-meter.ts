 ...
 *
 * БЕЗ `import "server-only"`, И ЭТО НЕ НЕДОСМОТР. Его нет и в `src/lib/
 * db.ts`, который этот файл и подключает, по той же причине: голое имя
 * `server-only` умеет разрешать только сборщик Next, а `db.ts` тянут ещё и
 * служебные скрипты на `tsx` (`prisma/*.ts`, `scripts/*.ts`). Заход 7.222
 * сначала поставил его сюда, и `verify` упал `MODULE_NOT_FOUND` на цепочке
 * `search/sources.ts → db.ts → db-read-meter.ts`.
 *
