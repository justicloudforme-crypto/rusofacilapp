 ...
 *
 * В ВЕБЕ НЕ МЕНЯЕТСЯ НИЧЕГО — ТАК БЫЛО ДО 18.09.2026. `nativeShell: false`
 * по-прежнему возвращает ровно то, что возвращал `accessMarkFor`, знак в
 * знак, и это отдельное утверждение в `src/lib/access-marks.test.ts`.
 *
 * НО ПОВЕРХНОСТЕЙ, КОТОРЫЕ ЭТУ ВЕТКУ ПРОСЯТ, ОСТАЛОСЬ МЕНЬШЕ. Решением
 * владельца от 18.09.2026 браузер и оболочка говорят про доступ ОДНО И
 * ТО ЖЕ: сначала у сетки тем словаря (долг 257), следом у плиток пазлов
 * и полосы уровней игр. Эти поверхности зовут правило с
 * `nativeShell: true` в ОБОИХ местах — и это ИМЯ ОПЦИИ, а не место:
 * опция включает «корона это сорт, замок это состояние», а имя ей
 * досталось от места, где правило появилось впервые.
 *
 * Почему поменяли: замер по 12 экранам 18.09.2026 (три роли × две
 * локали × оболочка/браузер, сумма по 4107 плиткам на роль) показал, что
 * у роли «доступ по коду» в оболочке 580 замков, а в браузере 0, и что
 * Premium в браузере не видел НИ ОДНОЙ из своих 580 корон — то есть
 * граница премиального материала не показывалась именно тому, кто за неё
 * платит. Держится сторожем `check:access-signs`, направления «ИГРЫ:
 * браузер говорит то же, что оболочка» и «ИГРЫ: у Premium замков нет
 * нигде», с подсадкой в обе стороны.
 */
