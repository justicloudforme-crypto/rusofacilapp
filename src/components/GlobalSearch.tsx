...
export default function GlobalSearch({
  ...
  return (
    <>
      {/* 36x36, а не 44x44, и это НЕ забытая правка тап-таргета.
          06.09.2026 эта кнопка в заходе 7.131 была увеличена 36 -> 44 вместе
          с элементами вводной деки — и уронила `CI / Playwright E2E` на
          e2e/navbar-signed-in.spec.ts (`/ru/profile`, PR #203). Замер на базе
          в форме CI, ширина 640, вошедший учитель с подпиской:

            main   — правый край правого кластера шапки 631.48 при вьюпорте 640
                     (запас 8.52 px), кластер 238.48
            ветка  — 639.48 (запас 0.52 px), кластер 246.48

          Ряд шапки в полосе 640–767 УЖЕ шире своего контейнера: контентная
          коробка кончается на 616, и ряд живёт за счёт правого padding-а
          (24 px). На main из этого буфера съедено 15.48, ветка довела до
          23.48 — то есть 8 из 8.52 оставшихся пикселей. Ровно 0.52 px не
          переживают чужую растеризацию шрифта: в Linux-Chromium из CI PT Sans
          отдаёт целочисленные advance-ы, «Мой профиль» шире на пару пикселей,
          и документ выезжает за вьюпорт. На `/es` («Mi perfil») запас 18.86 px
          — и `/es/profile` в том же прогоне CI зелёный.

          Поэтому 44x44 здесь нельзя ввести правкой одной кнопки: сначала
          полосе 640–767 нужен бюджет ширины (долг), а это уже глобальная
          правка шапки — её нельзя делать, не доказав, что дельта одинакова
          для обеих групп замороженного эксперимента. */}
      <button
        type="button"
        onClick={openWindow}
