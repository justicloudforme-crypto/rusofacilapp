 ...
 *
 * Число «без записи» не обязано быть нулём: после захода 7.164 их 235, и
 * ни одна не дефект кода. Разложение (PROGRESS.md 7.164): 231 — клип
 * синтезирован и ОТКЛОНЁН аудитом, а отклонённое на прод не пишется и по
 * кругу не перегенерируется; 3 — отложены до 26.09, потому что их запись
 * попала бы в серверный HTML замороженной `song-katyusha`; 1 — текст с
 * кавычками «», которые санитайзер вырезает, из-за чего сырой текст
 * кнопки не совпадает с текстом клипа (долг 125). Поэтому гейт сравнивает с ЗАКРЕПЛЁННЫМ ЧИСЛОМ (BASELINE
 * ниже, приём `check:brand`): стало больше — падение; стало меньше —
 * тоже падение, чтобы число в PROGRESS.md не расходилось с продуктом
 * молча.
 *
 * Запускать:
 ...
 * долга 122 (заход 7.164). Ключ — поверхность, значение — [кнопок, с
 * записью]. Прошлые числа этой таблицы — в PROGRESS.md 7.163.
 */
const BASELINE: Record<string, [number, number]> = {
  "урок: слайды": [679, 638],
  "урок: словарь": [1930, 1921],
  "урок: произношение": [720, 718],
  "урок: грамматика": [737, 734],
  "урок: чтение вслух": [991, 983],
  "урок: алфавит": [33, 33],
  "урок: упражнение (аудирование)": [250, 250],
  "урок: упражнение (диктант)": [232, 232],
  "урок: упражнение (чтение)": [119, 119],
  "медиа: ключевая лексика": [1312, 1172],
  "глоссарий: термин": [119, 94],
  "глоссарий: пример": [240, 237],
  "карточки: слово": [5771, 5767],
