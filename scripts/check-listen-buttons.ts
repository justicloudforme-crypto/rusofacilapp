 ...
 *
 * Число «без клипа» не обязано быть нулём: после 7.170 их 220, и ни одно
 * не дефект кода. Разложение (PROGRESS.md 7.170): 3 — «груша»,
 * «расцветать», «яблоня» со `song-katyusha`, отложены до 26.09
 * заморозкой; 217 — места омографов, у которых нет годной вырезки из
 * озвучки их собственного предложения. Изолированным клипом омограф не
 * озвучивается никогда — он дал бы чужое ударение; вырезку по месту
 * получили 466 мест: 46 записаны в 7.168 и 420 в 7.170, все — после
 * того как владелец прослушал каждую. Поэтому гейт сравнивает с
 ...
 *
 * ВНИМАНИЕ: числа BASELINE сняты ПОСЛЕ записи 420 вырезок омографов
 * (заход 7.170, долг 130) по ЖИВОЙ боевой базе. Поверхность «рассказы:
 * тап по слову» считается кодом `src/lib/story-word-pick.ts`, поэтому без
 * слитой правки её число разойдётся целиком.
 *
 * Запускать:
 ...
 *
 * У тапа по слову без клипа **637** мест, и все до одного — омографы:
 * клип изолированного слова дал бы им чужое ударение (правило в
 * `story-word-pick.ts`), а годной вырезки из озвучки их собственного
 * предложения пока нет. 46 мест вырезку получили в 7.168.
 */
const BASELINE: Record<string, [number, number]> = {
  ...
  "рассказы: тап по слову": [56642, 56425],
};
