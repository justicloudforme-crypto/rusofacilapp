 ...
 *
 * Число «без записи» не обязано быть нулём: 1187 кнопок озвучены не были
 * НИКОГДА (класс Б захода 7.163) — это решение владельца о цене, а не
 * дефект кода. Поэтому гейт сравнивает с ЗАКРЕПЛЁННЫМ ЧИСЛОМ (BASELINE
...
/**
 * Закреплённые числа переписи по боевой базе на 10.09.2026, ПОСЛЕ правки
 * 7.163. Ключ — поверхность, значение — [кнопок, с записью].
 */
const BASELINE: Record<string, [number, number]> = {
  "урок: слайды": [679, 502],
  "урок: словарь": [1930, 1919],
  ...
  "медиа: ключевая лексика": [1312, 513],
  "глоссарий: термин": [119, 91],
  "глоссарий: пример": [240, 182],
  "карточки: слово": [5771, 5765],
  "карточки: пример": [5771, 5766],
