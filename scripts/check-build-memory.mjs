 ...
 *
 * ЧИСЛО ВЗЯТО С LINUX, И ЭТО ВАЖНО. Первый замер 21.09.2026 делался на
 * macOS и дал пик 1,0–1,7 ГБ — из чего следовало бы, что до 8192 МБ далеко.
 * Прогон той же сборки в CI (ubuntu-latest) на том же коммите дал
 * **5018 МБ** суммы по дереву и **3835 МБ** у самого толстого процесса.
 * Разница втрое: `ps` на macOS показывает резидентную часть уже сжатой
 * памяти, а на Linux — настоящую. Вывод для протокола: пик сборки надо
 * мерить на Linux, замер с ноутбука занижает.
 *
 * Отсюда и порог. Измеренный пик 5018 МБ; 6400 — на 1382 МБ (27 %) выше
 * измеренного, чтобы обычный разброс прогона не красил CI зря, и на
 * 1792 МБ (22 %) ниже 8192 МБ контейнера Vercel, чтобы сторож срабатывал
 * РАНЬШЕ, чем контейнер. Прежнее значение 5600 оставляло запас 582 МБ —
 * это 10 %, и такой гейт краснел бы от шума.
 *
 * Верхнюю границу самого ПРОЦЕССА держит не этот порог, а потолок кучи
 * 2560 МБ: 3835 МБ его RSS — это 2560 кучи плюс около 1,2 ГБ внекучевого
 * (код, буферы сериализации кеша), и расти сверх потолка V8 не может.
 */
export const BUDGET_MB = 6400;
