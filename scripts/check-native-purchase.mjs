...
const PLANTS = [
  ...
  {
    // ЧИСЛО СЧИТАЕТСЯ ОТ ЖИВОГО, А НЕ ПИШЕТСЯ СЮДА РУКАМИ. Заход 7.232
    // поднял версию до 9 — и подсадка, в которой стояла девятка, стала
    // заменой числа на него же, то есть переставала что-либо менять, и
    // сторож ловил её «не поймано». Такая подсадка умирает при каждом
    // подъёме версии, и умирает молча до самого CI.
    name: "versionCode ушёл вперёд того, чем оболочка себя называет",
    apply: (f) => ({
      ...f,
      gradle: f.gradle.replace(/versionCode\s+(\d+)/, (_, n) => `versionCode ${Number(n) + 1}`),
    }),
  },
  {
