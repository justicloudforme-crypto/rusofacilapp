...
const CANDIDATES = [
  ...
  // ГЛАВНАЯ СНИМАЕТСЯ НЕ СВЕРХУ, И ЭТО НЕ ВКУС (заход 7.191).
  //
  // Прежний кадр (`scrollY: 45`) показывал шапку, заголовок и все четыре
  // плитки чисел — в том числе плитку «OXXO / Pago en efectivo». Для
  // витрины Google Play это негодный кадр: внутри приложения оплата идёт
  // только через Google Play Billing (долг 79, заход 7.183), а упоминание
  // стороннего способа оплаты на скриншоте — это ровно то, за что
  // отклоняют карточку. Плитка при этом ЗАКОННА: она живёт по правилу
  // 7.117 и показывается только покупателю из Мексики. Дефекта тут нет,
  // негоден именно КАДР.
  //
  // Убрать плитку сдвигом кадра, оставив заголовок, нельзя арифметически.
  // Замер живого прода 13.09.2026 при ширине 360: полоса чисел занимает
  // 548,6…668,6 по странице, плитка OXXO — 616,6…668,6, шапка липкая и
  // держит верхние 65 px вьюпорта. Чтобы плитка ушла вниз за кромку,
  // нужен верх кадра на −23,5 — страница выше нуля не прокручивается. А
  // кадр, начинающийся ПОСЛЕ полосы (scrollY ≥ 669), заголовка уже не
  // содержит. Поэтому взят другой цельный экран той же страницы.
  //
  // 2083 — верх раздела «Cómo funciona»: заголовок и три шага целиком,
  // ничего не срезано ни сверху, ни снизу, и весь текст испанский.
  { slug: "01-home", url: "/es", wait: 2500, scrollY: 2083 },
