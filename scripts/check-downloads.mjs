 ...
 *  10. КЕШ ЛИСТОВ СТИЛЕЙ, ЗАВЕДЁННЫЙ ПРИ ПОТЕРЯННОЙ ГОНКЕ, УНОСИТСЯ
 *      (строка 317). Уборщик выхода стирает кеши целиком, а `put` заводит
 *      их заново; на телефоне вышедшего человека не должно остаться
 *      кеша, которого он не заводил. Поймано живой пробой на CI
 *      25.09.2026 («после выхода кешей rf-pages*: ожидалось 0, получено 2»).
 *
...
export function violations(sources) {
  ...
  if (!/const madeSheetCache = await keepSheetsBeside\(deps\.caches/.test(saveClient)) {
    bad.push(
  ...
  // 10 — свой кеш листов стилей уносится при потерянной гонке
  if (!/if \(madeSheetCache\) \{/.test(saveClient) || !/await deps\.caches\.delete\(deps\.names\.sheets\)/.test(saveClient)) {
    bad.push(
      `${SAVE_CLIENT}: кеш листов стилей, заведённый при потерянной гонке, не уносится — на телефоне вышедшего человека останется кеш, которого он не заводил (строка 317)`,
    );
  }
  if (!/const existed = \(await store\.keys\(\)\)\.includes\(cacheName\)/.test(saveClient)) {
    bad.push(
...
async function plant() {
  ...
  add(
    "подсадка: заведённый кеш листов стилей остаётся после потерянной гонки (строка 317)",
    SAVE_CLIENT,
    (s) => s.replace("if (madeSheetCache) {", "if (false) {"),
    "не уносится — на телефоне вышедшего человека",
  );
  add(
    "подсадка: сохранение перестало отличать свой кеш листов от чужого (строка 317)",
    SAVE_CLIENT,
    (s) => s.replace("const existed = (await store.keys()).includes(cacheName);", "const existed = false;"),
    "не отличает «кеш листов стилей завели мы»",
  );
  add(
