...
//
// ДВА СЛОЯ, А НЕ ОДИН (долг 139 закрыт 12.09.2026, заход 7.179). Всё,
...
// создан 12.09.2026, и оба артефакта собраны и проверены им впервые.
//
...
export function judgeSigning({ gradle, gitignore, keyFiles, pbxproj }) {
  ...
  //    12.09.2026. Первая редакция этого правила валила ЛЮБОЕ совпадение
...
// АРТЕФАКТАХ (12.09.2026, все три собраны, а не выдуманы):
...
export function judgePackage(inputs, expectSha256) {
  ...
  for (const { name, text } of inputs) {
    ...
    //    так выглядит неподписанный артефакт, который до 12.09.2026
...
// АРТЕФАКТАМ, СОБРАННЫМ 12.09.2026. Не выдуманы и не сокращены по
...
function plant() {
  ...
  const healthy = {
    ...
    // НАСТОЯЩИЙ обход, а не пустой список: с 12.09.2026 у владельца есть
