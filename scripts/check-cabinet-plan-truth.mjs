 ...
 *      роли.
 *   5. Блок «Код доступа» закрыт признаком верхнего разряда доступа
 *      (`isPremiumUser`) — то есть невиден сотруднику, владельцу и
 *      пожизненному.
...
export function judge(sources) {
  ...
  // --- 5. Блок «Код доступа» закрыт верхним разрядом доступа ----------
  if (!code.includes("{!isPremiumUser && (")) {
    problems.push(
      `${PAGE}: блок «Код доступа» показывается всем — в том числе сотруднику, владельцу и ` +
        `пожизненному, у которых доступ уже полный, а маршрут погашения отказывает при tier !== "free"`,
    );
  }
...
export async function main() {
  ...
  if (plant) {
    ...
    const plants = [
      ...
      ["блок «Код доступа» снова показывается всем",
        { [PAGE]: sources[PAGE].replace("      {!isPremiumUser && (\n      <section", "      <section") }],
  ...
  console.log(
    "check:cabinet-plan-truth — признак «доступ по роли» один на весь кабинет и спрашивается обоими " +
      ...
      "тому, у кого доступ по роли; блок «Код доступа» закрыт верхним разрядом доступа, а его подсказка " +
