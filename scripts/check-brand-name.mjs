...
const IDENT_ALLOWED = new Map([
  ...
  // 7.211, задача 2. Проба обёртки хранилища прогоняет НАСТОЯЩИЕ ключи
  // продукта, а у трёх из десяти приставка `rusofasil:` подлинная (те же
  // ключи, что цитирует перепись выше). Подставить сюда `rusofacil`
  // значило бы сличать «до и после» на выдуманных ключах.
  ["src/lib/safe-storage.test.ts", 3],
...
const ALLOWED = new Map([
  ...
  [
    "src/lib/story-author.ts",
    {
      hits: 1,
      why:
        "САМО ПРАВИЛО починки написания: `BRAND_TYPO` (долг 182). Написать " +
        "правило, не назвав неверное написание, нельзя по построению. " +
        "Было 2: второе попадание жило в комментарии, который 7.196 " +
        "переписал — там больше не нужно называть старое написание, чтобы " +
        "объяснить правило.",
    },
  ],
  [
  ...
  const controls = [
    ...
    },
    {
      ...
      plant: function () {
        this.restore = swap(
          "src/lib/story-author.ts",
          // 7.196: русская ветка теперь ещё и переводит маркер оригиналов,
          // поэтому строка выхода по локали выглядит иначе. Подсадка
          // проверяет то же самое, что и раньше: починка написания обязана
          // стоять ДО выхода по локали, иначе русская карточка её не получит.
