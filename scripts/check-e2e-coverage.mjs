 ...
 *
 *   431 -> 443 (14.09.2026, PROGRESS.md 7.195): две спеки захода про знаки
 *       словаря и форму входа — `e2e/native-dictionary-signs.spec.ts`
 *       (четыре пробы: плашка ровно одна у гостя, она же у вошедшего без
 *       подписки, ни одной плитки «0 слов» при непустом банке, и
 *       отрицательный контроль «в вебе ничего этого нет») и
 *       `e2e/login-keeps-email.spec.ts` (по две пробы на локаль: адрес
 *       остаётся после неверного пароля, и отрицательный контроль «пустая
 *       форма остаётся пустой»). 4 + 4 = 8 проб в двух проектах = 16... —
 *       но НЕ 16: `--list` до и после дал 461 и 473, то есть +12, и число
 *       здесь снято этой разностью, а не сложением. Расхождение со
 *       сложением объясняется тем, что `voice-ios-shape` берёт только
 *       одну спеку (`testMatch`), и новые файлы в него не попадают.
 *
