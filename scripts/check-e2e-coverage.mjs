 ...
 * projects, so 2 × 2 × 2 = 8. Raised from 89 to 91 on 03.09.2026, when
 ...
 * puzzle in each locale — in each of the two projects they run in, so
 * 4 x 2 = 8.
 ...
 *
 * Поднят 221 -> 223 на 06.09.2026 (PROGRESS.md 7.129, часть 5): шестой
 * случай в той же спеке — ВОШЕДШИЙ пользователь. Он закрывает пробел
 * 7.128 части 5 №7 (все десять прогонов шли анонимно) и проверяет две
 * вещи разом: состав выдачи одинаков у анонима, у вошедшего без подписки
 * и у подписчика, а пометка «нужна подписка» есть у первых двух и
 * пропадает у третьего. 1 x 2 проекта = 2. `playwright test --list` —
 * 223 в 24 файлах.
 *
 * Поднят 223 -> 235 на 06.09.2026 (PROGRESS.md 7.130, часть 1, долг 52):
 * новая спека e2e/search-demand-log.spec.ts — шесть случаев в каждом из
 * двух проектов, 6 x 2 = 12. Четыре из них проверяют, что запись спроса
 * уходит на КАЖДОМ выходе из окна поиска (Escape, переход по строке
 * выдачи, уход на другой адрес, закрытие вкладки), а два оставшихся —
 * отрицательные контроли к ним: «Escape и следом закрытие вкладки»
 * обязаны дать одну строку, а не две, и окно, открытое и закрытое без
 * ввода, — ни одной. `playwright test --list` — 235 в 25 файлах.
 *
 * Поднят 235 -> 275 на 06.09.2026 (PROGRESS.md 7.131, часть 3): новая
 * спека e2e/intro-deck-layout.spec.ts — вводная дека на пяти реальных
 * ширинах телефона (320, 360, 375, 390, 393) в двух локалях, по два
 * утверждения на ширину: карточка слайда не режет собственное
 * содержимое и каждый кликаемый элемент деки не меньше 44x44.
 * 5 x 2 x 2 = 20 тестов в каждом из двух проектов, 20 x 2 = 40.
 ...
 * сразу, +12:
 *
 *   +8  e2e/global-search-enter.spec.ts — Enter в поиске. Четыре случая
 *       (переход по первому результату на `es` и на `ru`; пустая выдача
 *       никуда не уводит и не перезагружает страницу; Enter, нажатый до
 *       прихода выдачи) в каждом из двух проектов, 4 x 2 = 8. До этого
 *       дня на поле не висело ни одного обработчика `keydown`, и Enter не
 *       делал ничего — замер 64 конфигураций на живом проде.
 *   +4  e2e/global-search.spec.ts — два случая про адрес строки выдачи:
 *       полное название одного пазла уводит на сам пазл, а не на хаб игр,
 *       и закрытый пазл несёт пометку про подписку при собственном
 *       адресе. 2 x 2 = 4.
 *
 *  +36 e2e/intro-deck-pager.spec.ts — переключатель слайдов вводной деки:
 *       две локали x четыре ширины (320/360/390/768) x ДВА КОРНЕВЫХ КЕГЛЯ
 *       (16 и 20 px) на «одна строка и элементы >=44x44», 2 x 4 x 2 = 16,
 *       плюс по одному поведенческому случаю на локаль (число «7 / 10»,
 *       единственный aria-current, стрелки клавиатуры, свайп и порог
 *       свайпа), 16 + 2 = 18, в двух проектах — 36. Кегль здесь не
 *       украшение: при 16 px полоса на прежней вёрстке ложилась в 5 + 5, а
 *       жалоба владельца (4 + 4 + 2) воспроизводится только при 18–20 px,
 *       потому что кнопка задана в rem, а потолок полосы был в пикселях.
 *
 *  +24 e2e/access-code.spec.ts — экран ввода кода доступа (долг 90,
 *       PROGRESS.md 7.147): шесть случаев (успех и открывшийся платный
 *       пазл; неизвестный код; уже погашенный; просроченный; отозванный;
 *       человек, у которого доступ уже есть) x две локали x два проекта,
 *       6 x 2 x 2 = 24. Число снято ПРОГОНОМ, а не сложением: отчёт
 *       прогона дал 403 исполнения при 0 пропусков.
 *   403 -> 415 (08.09.2026, PROGRESS.md 7.150): испанское название
 *       рассказа — `e2e/story-spanish-title.spec.ts`, шесть проб
 *       (испанское название первой строкой; рассказ без него; замороженный
 *       рассказ; вся русская локаль; каталог и серверный указатель; поиск
 *       по обоим названиям) x два проекта = 12. Число снято ПРОГОНОМ в
 *       форме CI: 415 исполнений, 0 пропусков, 0 flaky.
 *   415 -> 419 (09.09.2026, PROGRESS.md 7.151): код, вставленный из
 *       мессенджера, — новый случай в `e2e/access-code.spec.ts` (тире
 *       U+2013, неразрывный дефис, мягкий перенос и нулевая ширина через
 *       НАСТОЯЩУЮ форму, плюс подпись тарифа «доступ по коду-приглашению» в
 ...
 *
 *   431 -> 449 (14.09.2026, PROGRESS.md 7.195): две новые спеки захода —
 *       `e2e/native-dictionary-signs.spec.ts` (плашка ровно одна у гостя;
 *       она же у вошедшего без подписки; ни одной плитки «0 слов» при
 *       непустом банке; отрицательный контроль «в вебе ничего этого нет»)
 *       и `e2e/login-keeps-email.spec.ts` (адрес остаётся после неверного
 *       пароля и отрицательный контроль «пустая форма остаётся пустой», по
 *       две пробы на локаль) — плюс один контроль пары в
 *       `e2e/search-deep-link.spec.ts` («карточку C1 аноним в выдаче не
 *       получает вовсе») взамен шести проб доводки, которых в анонимной
 *       роли и быть не может: фикстура получила строки уровня C1, а C1
 *       анониму не отдаётся.
 *
 *       Число снято ПРОГОНОМ `--list` по обеим сторонам, а не сложением:
 *       на `main` 445 в 35 файлах, на ветке 463 в 37, разность **+18**.
 *       Пол поднят на ту же разность.
 *
 *   449 -> 455 -> 475 (18.09.2026, PROGRESS.md 7.208): три новые спеки
 *       захода — `e2e/word-game-crown-and-lock.spec.ts` (замок рядом с
 *       короной: доступ по коду и Premium по две локали плюс отрицательный
 *       контроль веба = 5), `e2e/vocabulary-zero-denominator.spec.ts`
 *       (дробь «0 из 0» и положительный контроль «знаменатель на месте»,
 *       по две пробы на локаль = 4) и
 *       `e2e/known-server-opinion-wins.spec.ts` (мнение сервера сильнее
 *       заявления браузера = 1), итого 10 в двух проектах = **20**.
 *
 *       Число снято ПРОГОНОМ `--list` по обеим сторонам, а не сложением:
 *       без этих файлов 477 в 39 файлах, с ними 497 в 42, разность
 *       **+20**. Пол поднят на ту же разность: 455 -> **475**.
 *
 * ПОЧЕМУ ПОЛ НИЖЕ, ЧЕМ `--list`. `--list` на 14.09.2026 печатает 461 до
 * этого захода при поле 431: пол — нижняя граница, а не равенство, и
 * поднимается он РАЗНОСТЬЮ, а не подстановкой измеренного числа. Класть
 * сюда 461 значило бы задним числом объявить проверенным то, чего этот
 * заход не измерял прогоном.
 */
const MIN_EXECUTED_TESTS = 475;
...
const ALLOWED_SKIPS = new Map([
  ...
  [
    "e2e/no-browser-voice.spec.ts:107",
    // Тот же пропуск и по тому же ИЗМЕРЕННОМУ условию, что строкой выше:
    // рассказ фикстуры спрашивается у сервера (`storyFixtureShape`), и в
    // CI он отвечает 200 — значит там не срабатывает ни разу. Две другие
    // поверхности этой же пробы (урок и термин глоссария) фикстуры не
    // требуют и идут всегда, поэтому вопрос «заговорил ли синтез» без
    // ответа не остаётся никогда.
    "база под сервером не в форме CI: фикстуры рассказов в ней нет (PROGRESS.md 7.217, долг 118)",
  ],
]);
