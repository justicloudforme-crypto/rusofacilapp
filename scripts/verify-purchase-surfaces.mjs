 ...
 * без него ни одна из трёх ролей не войдёт), гоняется своя доля, и на
 * ПЕРВОЙ доле — обязательная подсадка. Подсадка живёт на каждом экране,
 * поэтому платить за неё во всех долях значило бы платить за одно и то же.
 *
...
export async function main() {
  ...
  try {
    ...
    // Подсадка — только на первой доле: она живёт на КАЖДОМ экране, и
    // повторять её в каждой доле значит платить трижды за одно.
    let plant = { status: 0 };
    if (shardArg.startsWith("--shard=1/")) {
      plant = spawnSync(
        process.execPath,
        ["scripts/check-rendered-purchase-surfaces.mjs", `--base=${BASE}`, "--plant", "--limit=4"],
        { stdio: "inherit" },
      );
    }
    console.log(`  доля заняла ${Math.round((Date.now() - started) / 1000)} с`);
    return (run.status ?? 1) || (plant.status ?? 1);
  } finally {
    stop();
  }
}
