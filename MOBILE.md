...
## Адрес сервера: боевой по умолчанию, локальный — явным режимом

**Переключать на продакшен больше нечего: он и есть молчаливое значение.**
С 09.09.2026 (долг 109) `capacitor.config.ts` без единой переменной
окружения подставляет `https://rusofacilapp.com`. До этого там по
умолчанию стоял `http://192.168.1.69:3000` — адрес ноутбука, — и он
доезжал до **релизного** пакета: в `app-release-unsigned.apk` от
09.09.2026 внутри `assets/capacitor.config.json` лежала строка
`"url": "http://192.168.1.69:3000"`, а в скомпилированном манифесте —
`usesCleartextTraffic=true`.

Живой перезапуск с ноутбука никуда не делся, но стал **явным режимом** и
требует ДВУХ переменных сразу:

```bash
CAPACITOR_LIVE_RELOAD=1 CAPACITOR_SERVER_URL=http://192.168.1.69:3000 npx cap sync android
```

Одной `CAPACITOR_SERVER_URL` мало: на адрес `http://` или в частной сети
(`10.*`, `172.16–31.*`, `192.168.*`, `127.*`, `169.254.*`, `localhost`,
`*.local`) без второй переменной конфиг **бросает**, и `cap sync` падает —
пакет с таким адресом внутри просто не собирается.

`android:usesCleartextTraffic="true"` переехал из
`android/app/src/main/AndroidManifest.xml` в
`android/app/src/debug/AndroidManifest.xml`. Набор `debug` мерджер
подмешивает только в отладочный вариант, поэтому в релизном варианте
взять этот атрибут неоткуда — это устройство сборки, а не
договорённость.

Третья линия — сторож `npm run check:native-release-safety`. Он читает
**собранный релизный APK** (`aapt2 dump xmltree` + `assets/capacitor.config.json`
из пакета), а не исходники, и гоняется в
`.github/workflows/android-debug.yml` на неподписанном `assembleRelease`.
Его лёгкий слой (исходники) стоит в `npm run verify` и в `ci.yml`.
...
- `capacitor.config.ts` — конфиг, remote-URL режим, сплэш/статус-бар плагины,
  `server.allowNavigation` (LAN-хост + `rusofacilapp.com`/`*.rusofacilapp.com`) —
  без этого Capacitor может решить, что обычный редирект после логина/
  регистрации (303 на `/api/auth/login`) — это переход на "внешний" сайт,
  и открыть системный браузер вместо того, чтобы остаться в WebView.
  Добавлено 2026-08-25 по реальному репорту с устройства; окончательно
  подтвердить можно только на телефоне после `npx cap sync`.
...
## Плеер в системной шторке уведомлений (media session)

Веб-версия (`src/components/stories/StoryText.tsx`) уже полностью использует
`navigator.mediaSession` — metadata, playbackState, обработчики play/pause/
seek/prev/next. Этого **достаточно для Chrome**, но **не для обычного
Android System WebView**, на котором работает Capacitor: в отличие от
Chrome, системный WebView не показывает медиа-уведомление из веб-страницы
автоматически — нужен нативный мост к настоящему Android `MediaSession` +
foreground-уведомлению.

Добавлен `@capgo/capacitor-media-session` (версия синхронизирована с
Capacitor 8, лицензия MPL-2.0 — коммерчески безопаснее GPL-аналогов) —
JS-обёртка `src/lib/native-media-session.ts` (тот же паттерн no-op-на-вебе,
что и `haptics.ts`/`notifications.ts`), подключена в `StoryText.tsx`
**параллельно** с уже существующим `navigator.mediaSession` (не вместо
него — веб/PWA-поведение не тронуто).

**Что сделано здесь**: установка пакета, `npx cap sync` (плагин подхватился
и под iOS, и под Android — 9 плагинов вместо 8), JS-мост, типы/линт/сборка
проверены.

**Что осталось (нужен Android Studio, здесь недоступен)**: реальная сборка
и проверка на устройстве — увидеть, что при прослушивании рассказа
появляется управление в шторке уведомлений и на экране блокировки. Если на
устройстве управление всё ещё не появляется — скорее всего плагину нужно
явное разрешение `POST_NOTIFICATIONS` (Android 13+) или дополнительная
настройка `AndroidManifest.xml` под конкретную нативную реализацию плагина;
смотрите `android/app/src/main/java/.../` после `cap sync` — Capacitor уже
сгенерировал там нужные Java/Kotlin-файлы плагина, их не нужно писать
руками.
