 ...
 *
 * Условие перехода к следующей пробе — «закрытых строк нет», а НЕ «ответ
 * пуст», и это разные вопросы. До 08.09.2026 цикл шёл дальше по
 * `total === 0`, то есть ровно тогда, когда объекта нет вовсе; на полной
 * базе первая проба находит пазл (`total > 0`) и он БЕСПЛАТЕН, цикл на
 * ней останавливался и падал на `locked.first()`. Вторая проба, ради
 * которой обе строки и написаны, не спрашивалась ни разу.
 *
 * Пустой ответ теперь падение, а не пропуск. Каждая проба — полное
 * название объекта, который в базе обязан быть: ноль совпадений значит
 * не «здесь нечего проверять», а «база не та, на которую написан тест»,
 * и молча пропускать это — тот самый пропуск по данным, которого
 * `check:e2e-coverage` не видит (PROGRESS.md 7.141, слепая зона Б4).
 * Порядок проб держит это истинным в обеих базах: в фикстуре CI первая
 * проба сразу даёт закрытую строку и до второй дело не доходит, а в
 * полной `dev.db` обе строки существуют.
 */
test("закрытый пазл: пометка про подписку, свой адрес, и без подписки не открывается", async ({
  ...
  for (const probe of probes) {
    ...
    //
    // И ответ именно НА ЭТУ пробу, а не первый попавшийся: окно поиска
    // успевает сходить за выдачей раньше, чем в поле окажется текст, и
    // без сверки адреса `total` читался бы у чужого запроса. Замерено
    // подсадкой 08.09.2026: контроль «первая проба вернула пусто» без
    // этой строки давал 2 прохода из 3 там, где обязан падать всегда.
    const [response] = await Promise.all([
      page.waitForResponse(
        (r) =>
          r.url().includes("/api/search") &&
          r.status() === 200 &&
          decodeURIComponent(r.url()).includes(probe),
      ),
      page.getByRole("searchbox").fill(probe),
    ]);
    const total = ((await response.json()) as { total: number }).total;
    expect(
      total,
      `проба «${probe}» не нашла ни одного объекта — это не повод пропустить пробу, это другая база`,
    ).toBeGreaterThan(0);

    const rows = page.locator('[data-testid="search-section-game"] [data-testid="search-result"]');
    // Ответ пришёл — значит строки будут; ждём их появления, иначе счёт
    // ниже вернул бы ноль просто потому, что React ещё не отрисовал.
    await expect(rows.first()).toBeVisible();

    const locked = rows.filter({ has: page.locator('[data-testid="search-result-locked"]') });
    // Вот здесь — и только здесь — законно перейти к следующей пробе:
    // объект найден, но он бесплатен, а тест про закрытый.
