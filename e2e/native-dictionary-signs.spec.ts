...
test("ни одна плитка не пишет «0 слов», когда в банке по этому пересечению есть карточки", async ({ page, context }) => {
  ...
  // Тот же селектор, что и в отрицательном утверждении веба ниже, но здесь
  // он обязан НАЙТИ — иначе опечатка в нём дала бы зелёный на обоих концах.
  await expect(page.locator(TILE_MARK)).toHaveCount(withBank);
});
