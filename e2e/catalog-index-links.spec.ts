...
for (const lang of ["es", "ru"] as const) {
  ...
  test(`${lang}: каталог рассказов даёт ссылку на каждый рассказ без JavaScript`, async ({ page }) => {
    ...
    // упоминает ссылкой, включая карточки каталога.
    //
    // ЗДЕСЬ БЫЛО «пусто и там и там — тоже согласие», и это неверно.
    // Замер 08.09.2026 в форме CI: `indexed=0`, `all=0` в 4 исполнениях из
    // 4, то есть на пути мержа обе строки ниже сравнивали два пустых
    // списка и не проверяли ничего (PROGRESS.md 7.149, долг 94). Пустой
    // каталог — это состояние, о котором тоже есть что утверждать:
    // `StoryLinkIndex` возвращает `null`, когда группировать нечего, так
    // что секция обязана стоять ТОГДА И ТОЛЬКО ТОГДА, когда рассказы есть.
    // Это утверждение краснеет в обе стороны и на любой базе.
    const indexPresent = (await page.locator('[data-testid="story-link-index"]').count()) > 0;
    console.log(`  /${lang}: ссылок на рассказы на странице ${all.length}, в указателе ${indexed.length}`);
    expect(
      indexPresent,
      `рассказов на странице ${all.length}, а секция-указатель ${indexPresent ? "есть" : "отсутствует"}`,
    ).toBe(all.length > 0);
