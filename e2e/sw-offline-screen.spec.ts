...
test("при живой сети офлайн-заглушка не показывается ни разу, и прибор это умеет видеть", async ({ page }) => {
  ...
  //
  // СПИСОК СОХРАНЁННОГО ИЗ ЗАМЕРА ИСКЛЮЧЁН, И ЭТО НЕ ПОСЛАБЛЕНИЕ (7.230).
  // Утверждение здесь — про СОБСТВЕННЫЕ слова каркаса: они обязаны быть
  // одной локали. А строки списка — это НАЗВАНИЯ сохранённых страниц, и
  // язык у них тот, на котором страница написана: человек, открывший
  // русский урок, обязан увидеть в списке «Урок 1», а не перевод,
  // которого у нас нет. Обход этой пробы ходит и по русским адресам,
  // поэтому кириллица в списке — правильный ответ, а не дефект.
  const text = await page.evaluate(() => {
    const list = document.querySelector("[data-saved-list]") as HTMLElement | null;
    const was = list?.style.display ?? "";
    if (list) list.style.display = "none";
    const seen = document.body.innerText;
    if (list) list.style.display = was;
    return seen;
  });
